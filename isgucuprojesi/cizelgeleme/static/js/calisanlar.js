...
$(document).ready(function() {
    ...
    $("#calisanArama").on("keyup", function() {
        ...
        $(".calisan-kart").each(function() {
            ...
            var yetkinlik = $(this).find(".yetkinlik-seviyesi").text().toLowerCase();
            
            // Çalışan adı veya yetkinlik arama değerini içeriyorsa göster, içermiyorsa gizle
            if (calisanAdi.indexOf(value) > -1 || yetkinlik.indexOf(value) > -1) {
                $(this).show();
            } else {
                $(this).hide();
            }
    ...
    function checkNoResults() {
        if ($(".calisan-kart:visible").length === 0) {
            ...
            if ($("#noResults").length === 0) {
                $("#calisan-listesi-container").append('<div id="noResults" class="col-12 text-center mt-4"><h4>Sonuç bulunamadı</h4></div>');
            }
        } else {
